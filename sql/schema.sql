...
CREATE TABLE IF NOT EXISTS bot_ai_runtime_config (
    ...
    max_tokens INT UNSIGNED NULL DEFAULT 1024 COMMENT '单次最大生成Token数',
    temperature DECIMAL(3,2) NULL DEFAULT NULL COMMENT '采样温度 0.00-2.00',
    top_p DECIMAL(3,2) NULL DEFAULT NULL COMMENT '核采样阈值 0.00-1.00',
    presence_penalty DECIMAL(3,2) NULL DEFAULT NULL COMMENT '存在惩罚 -2.00-2.00',
    frequency_penalty DECIMAL(3,2) NULL DEFAULT NULL COMMENT '频率惩罚 -2.00-2.00',
...
CREATE TABLE IF NOT EXISTS bot_tool_config (
    ...
    tool_type VARCHAR(16) NOT NULL DEFAULT 'builtin' COMMENT '工具类型：builtin/http/python',
    ...
    python_code LONGTEXT NULL COMMENT 'Python 工具代码',
    python_entry VARCHAR(64) NULL COMMENT 'Python 入口函数名',
    python_allow_network TINYINT(1) NOT NULL DEFAULT 0 COMMENT 'Python 工具是否允许导入模块',
    python_timeout_seconds INT UNSIGNED NOT NULL DEFAULT 8 COMMENT 'Python 工具超时秒数',
