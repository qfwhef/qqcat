...
class MinecraftService:
    ...
    async def notify_restart(self, message: str, secret: str, group_ids: list[int] | None = None) -> dict:
        ...
        for group_id in target_groups:
            ...
            stats = build_message_delivery_stats(chunks)
            logger.info(
                "Minecraft通知发送: group_id=%s chunks=%s total_chars=%s max_chunk_chars=%s",
                group_id,
                stats.chunk_count,
                stats.total_chars,
            ...
            for index, chunk in enumerate(chunks, start=1):
                try:
                    ...
                    logger.error(
                        "Minecraft通知发送失败: group_id=%s chunk=%s/%s error=%s",
                        group_id,
                        index,
                        stats.chunk_count,
                        exc,
