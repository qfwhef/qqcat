  ...
  <div>
    <el-card class="page-card">
      <div class="messages-content">
          <div class="table-toolbar">
            <div>
              ...
              <div style="margin-top: 4px; color: #64748b; font-size: 13px">
                当前会话：
                <span v-if="selectedSession">{{ selectedSession.display_name }}（{{ selectedSession.session_id }}）</span>
          ...
          <el-form class="toolbar-form" :inline="true">
            ...
            <el-form-item label="关键词">
              <el-input v-model="filters.keyword" placeholder="内容/引用/工具名" style="width: 200px" clearable />
            </el-form-item>
            <el-form-item label="时间范围">
              <el-select v-model="timePreset" style="width: 160px" @change="applyTimePreset">
            ...
            <el-form-item label="是否引用">
              <el-select v-model="filters.is_reply" clearable style="width: 120px">
          ...
          <el-table :data="rows.items" @row-click="openDetail" @selection-change="handleSelectionChange">
            <el-table-column type="selection" width="48" />
            <el-table-column label="发送者昵称" min-width="160">
              <template #default="{ row }">{{ displaySenderName(row) }}</template>
            </el-table-column>
            <el-table-column label="内容" min-width="360">
              <template #default="{ row }">{{ summarize(row.content_text) }}</template>
            </el-table-column>
            <el-table-column prop="role" label="角色" width="100" />
            <el-table-column prop="sender_user_id" label="QQ号" min-width="130" />
    ...
    <el-drawer v-model="detailVisible" title="消息详情" size="55%">
      <el-descriptions v-if="currentRow" :column="2" border>
        <el-descriptions-item label="消息ID">{{ currentRow.id }}</el-descriptions-item>
        <el-descriptions-item label="平台消息ID">{{ currentRow.platform_message_id || '-' }}</el-descriptions-item>
        <el-descriptions-item label="会话ID">{{ activeTab === 'group' ? currentRow.group_id : currentRow.peer_user_id }}</el-descriptions-item>
        <el-descriptions-item label="发送者">{{ currentRow.sender_user_id || '-' }}</el-descriptions-item>
        <el-descriptions-item label="角色">{{ currentRow.role }}</el-descriptions-item>
        <el-descriptions-item label="消息类型">{{ currentRow.message_type }}</el-descriptions-item>
        <el-descriptions-item label="工具">{{ currentRow.tool_name || '-' }}</el-descriptions-item>
        <el-descriptions-item label="模型">{{ currentRow.model_name || '-' }}</el-descriptions-item>
        <el-descriptions-item label="引用消息">{{ currentRow.is_reply ? '是' : '否' }}</el-descriptions-item>
        <el-descriptions-item label="被引平台ID">{{ currentRow.quoted_platform_message_id || '-' }}</el-descriptions-item>
      </el-descriptions>
      ...
      <pre class="json-block">{{ formatJson(currentRow?.raw_message_json) }}</pre>
...
const timePreset = ref<'all' | 'last_24h' | 'last_7d' | 'last_30d' | 'older_7d' | 'older_30d' | 'custom'>('all')
...
const summarize = (value: string) => (value?.length > 80 ? `${value.slice(0, 80)}...` : value)
const displaySenderName = (row: any) => row.sender_card || row.sender_nickname || '-'
...
const applyTimePreset = async () => {
  ...
  if (timePreset.value === 'all') {
    ...
    filters.start_at = formatDateTime(new Date(now.getTime() - 24 * 60 * 60 * 1000))
    filters.end_at = ''
  } else if (timePreset.value === 'last_7d') {
    filters.start_at = formatDateTime(new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000))
    filters.end_at = ''
  } else if (timePreset.value === 'last_30d') {
    filters.start_at = formatDateTime(new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000))
    filters.end_at = ''
  } else if (timePreset.value === 'older_7d') {
    filters.start_at = ''
    filters.end_at = formatDateTime(new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000))
  } else if (timePreset.value === 'older_30d') {
    filters.start_at = ''
    filters.end_at = formatDateTime(new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000))
  }
  rows.page = 1
  await loadMessages()
}
...
const loadMessages = async () => {
  ...
  try {
    ...
    const data =
      activeTab.value === 'group' ? await adminApi.getGroupMessages(params) : await adminApi.getPrivateMessages(params)
...
const loadSessionMenus = async () => {
  const [groupRes, privateRes] = await Promise.all([adminApi.getGroupMessageSessions(), adminApi.getPrivateMessageSessions()])
...
const clearSelectedSession = async () => {
  ...
  selectedRows.value = []
  rows.page = 1
  await router.push(activeTab.value === 'group' ? '/messages/group' : '/messages/private')
}
...
const openDetail = async (row: any) => {
  currentRow.value =
    activeTab.value === 'group'
      ...
      : await adminApi.getPrivateMessageDetail(Number(filters.session_id), row.id)
...
const saveEdit = async () => {
  ...
  try {
    ...
    if (activeTab.value === 'group') {
      ...
      await adminApi.updateGroupMessage(Number(filters.session_id), editingRow.value.id, payload)
    } else {
      await adminApi.updatePrivateMessage(Number(filters.session_id), editingRow.value.id, payload)
    }
...
const performDelete = async (messageIds: number[]) => {
  ...
  if (activeTab.value === 'group') {
    ...
    await adminApi.deletePrivateMessages(Number(filters.session_id), messageIds)
  }
...
const handleBatchDelete = async () => {
  ...
  await ElMessageBox.confirm(`确认批量删除 ${selectedRows.value.length} 条消息吗？`, '批量删除', {
    type: 'warning',
  })
...
const applyRouteTab = async () => {
  const nextTab: 'group' | 'private' = route.path.includes('/messages/private') ? 'private' : 'group'
  const tabChanged = activeTab.value !== nextTab
  activeTab.value = nextTab
  const sessionId = route.query.session_id ? Number(route.query.session_id) : undefined
  filters.session_id = Number.isFinite(sessionId as number) ? sessionId : undefined
  const sessions = activeTab.value === 'group' ? groupSessions.value : privateSessions.value
  selectedSession.value = sessions.find((item) => Number(item.session_id) === filters.session_id) || null
